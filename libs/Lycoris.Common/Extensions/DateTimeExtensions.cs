...
namespace Lycoris.Common.Extensions
{
    ...
    public static class DateTimeExtensions
    {
        ...
        /// <returns></returns>
        public static DateTime MonthFirstDay(this DateTime time, string timeformat = "00:00:00") => DateTime.Parse($"{time.ToString($"yyyy-MM-01 {timeformat}")}");
        ...
        /// <param name="timeformat"></param>
        /// <returns></returns>
        public static DateTime MonthLastDay(this DateTime time, string timeformat = "23:59:59")
        {
            var _month = new List<int> { 1, 3, 5, 7, 8, 10, 12 };

            if (_month.Contains(time.Month))
                return DateTime.Parse($"{time.ToString($"yyyy-MM-31 {timeformat}")}");
            else if (time.Month != 2)
                return DateTime.Parse($"{time.ToString($"yyyy-MM-30 {timeformat}")}");
            else
            {
                if (time.Year % 400 == 0 || time.Year % 4 == 0 && time.Year % 100 != 0)
                    return DateTime.Parse($"{time.ToString($"yyyy-MM-29 {timeformat}")}");
                else
                    return DateTime.Parse($"{time.ToString($"yyyy-MM-28 {timeformat}")}");
            }
        }
