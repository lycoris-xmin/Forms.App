...
namespace Lycoris.Common.Extensions
{
    ...
    public static class EnumExtensions
    {
        ...
        /// <returns></returns>
        public static string? GetEnumDescription(this Enum value, Type type)
        {
            if (value == null)
                return "";

            return type.GetField(value.ToString())?.GetCustomAttributes(typeof(DescriptionAttribute), false).SingleOrDefault() is not DescriptionAttribute attribute ? value.ToString() : attribute.Description;
        }
        ...
        /// <returns></returns>
        public static List<T> GetEnumList<T>(this Type type) where T : Enum => Enum.GetValues(type).Cast<T>().ToList() ?? new List<T>();
    }
}
