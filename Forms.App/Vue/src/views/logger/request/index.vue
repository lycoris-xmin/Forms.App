...
import { fetchGetLoggerRequestDetail, fetchLoggerReqeustList } from '@/service/api';
...
const {
  ...
  apiFn: (...args) => {
    ...
    model.row = {};
    return fetchLoggerReqeustList(...args);
  },
  apiParams: {
    pageIndex: 1,
    pageSize: 20
...
function getColumns() {
  const cols = [
    ...
    },
    {
      ...
      customRender: ({ record }) => {
        return (
          <span class="request-url" onClick={() => copyToClipboard(record.url)}>
            {record.url}
          </span>
        );
      }
...
async function expandHandler(expanded, record) {
  ...
  try {
    //
    const { data: res, error } = await fetchGetLoggerRequestDetail(record.id);
        ...
        <template #expandedRowRender="{ record }">
          <div class="expand-body">
            <ADescriptions bordered :column="1">
              <ADescriptionsItem label="请求标识" :span="1">
                <div class="v-center flex">
                  <div class="content">
                    <p>{{ model.row[record.id]?.traceId || '' }}</p>
                  </div>
                  <div class="btn center flex">
                    <AButton
                      ...
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.traceId || '')"
                    >
                      复制
                    </AButton>
                  </div>
              ...
              <ADescriptionsItem
                v-if="model.row[record.id]?.requestHeaders && Object.keys(model.row[record.id].requestHeaders).length"
                label="请求头"
                :span="1"
              >
                <div class="v-center flex">
                  <div class="content">
                    <p>{{ jsonFormat(model.row[record.id].requestHeaders) }}</p>
                  </div>
                  <div class="btn center flex">
                    <AButton
                      size="small"
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.requestHeaders || '')"
                    >
                      复制
                    </AButton>
                  </div>
                ...
                <div class="v-center flex">
                  ...
                  <div class="btn center flex">
                    <AButton
                      ...
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.request || '')"
                    >
                      复制
                    </AButton>
                  </div>
              ...
              <ADescriptionsItem
                v-if="model.row[record.id]?.response && record.httpStatusCode < 500"
                label="响应内容"
                :span="1"
              >
                <div class="v-center flex">
                  ...
                  <div class="btn center flex">
                    <AButton
                      v-if="model.row[record.id]?.response !== model.row[record.id]?.exception"
                      size="small"
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.response || '')"
                    >
                      复制
                    </AButton>
                  </div>
                ...
                <div class="v-center flex">
                  ...
                  <div class="btn center flex">
                    <AButton
                      ...
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.exception || '')"
                    >
                      复制
                    </AButton>
                  </div>
                ...
                <div class="v-center flex">
                  ...
                  <div class="btn center flex">
                    <AButton
                      ...
                      type="primary"
                      ghost
                      @click="copyToClipboard(model.row[record.id]?.stackTrace || '')"
                    >
                      复制
                    </AButton>
                  </div>
              ...
              <ADescriptionsItem
                v-if="
                  !model.row[record.id]?.request &&
                  !model.row[record.id]?.response &&
                  !model.row[record.id]?.exception &&
                  !model.row[record.id]?.stackTrace
                "
              >
            ...
            <LoadingWrap
              v-if="model.row[record.id]?.loading || false"
              :loading="model.row[record.id]?.loading || false"
            ></LoadingWrap>
          </div>
        </template>
      </ATable>
