...
import {
  apiUrl,
  fetchDeleteDouyinPlanTaskComment,
  fetchGetDouyinPlanTaskCommentList,
  fetchRunRightNowDouyinPlanTaskComment
} from '@/service/api';
...
const {
  ...
  apiFn: fetchGetDouyinPlanTaskCommentList,
  apiParams: {
    pageIndex: 1,
    pageSize: 10
  },
...
async function batchDeleteHandler() {
  ...
  if (ids?.length) {
    const { data: res, error } = await fetchDeleteDouyinPlanTaskComment(ids);
    if (!error && res?.code === 0) {
      onBatchDeleted();
    }
  }
...
async function deleteHandler(id: string) {
  const { data: res, error } = await fetchDeleteDouyinPlanTaskComment([id]);
  if (!error && res?.code === 0) {
    onDeleted();
  }
}
...
function runRightNowButton(record: TableRecord) {
  if (rolePermission.hasOne(AppPermissions.DY.Comment.CREATE) && authStore.userInfo.isTenant) {
...
function deleteButton(record: TableRecord) {
  if (rolePermission.hasOne(AppPermissions.DY.Comment.DELETE)) {
...
async function runRightNowHandler(commentId: string) {
  const { data: res, error } = await fetchRunRightNowDouyinPlanTaskComment(commentId);
  if (!error && res?.code === 0) {
    window.$message?.success('发布操作成功');
    await getDataByPage(pagination.pageIndex);
  }
}
</script>
