...
import { apiUrl, fetchDeleteDouyinProduct, fetchDouyinShopName, fetchGetDouyinProductList } from '@/service/api';
...
const {
  ...
  apiFn: filter => {
    return fetchGetDouyinProductList({ ...filter });
  },
  apiParams: {
    pageIndex: 1,
    pageSize: 10
...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchDeleteDouyinProduct(ids);
    if (!error && res && res.code === 0) {
      onBatchDeleted();
    }
  }
...
async function deleteHandler(id: string) {
  window.$modal?.confirm({
    ...
    async onOk() {
      const { data: res, error } = await fetchDeleteDouyinProduct([id]);
...
function editButton(record) {
  if (!rolePermission.hasOne(AppPermissions.DY.Product.UPDATE)) {
    return '';
  }
...
function deleteButton(record) {
  if (!rolePermission.hasOne(AppPermissions.DY.Product.DELETE)) {
    return '';
  }
    ...
    >
      <template #extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :add-btn="rolePermission.hasOne(AppPermissions.DY.Product.CREATE)"
          :delete-btn="rolePermission.hasOne(AppPermissions.DY.Product.DELETE)"
          @add="handleAdd"
