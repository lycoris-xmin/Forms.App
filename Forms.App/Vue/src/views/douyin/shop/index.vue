...
import { fetchDeleteDouyinShop, fetchGetDouyinShopList } from '@/service/api';
...
const {
  ...
  apiFn: filter => {
    return fetchGetDouyinShopList({ ...filter });
  },
  apiParams: {
    pageIndex: 1,
    pageSize: 10
...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchDeleteDouyinShop(ids);
...
async function deleteHandler(id: string) {
  const { data: res, error } = await fetchDeleteDouyinShop([id]);
...
function editButton(record) {
  if (!rolePermission.hasOne(AppPermissions.DY.Shop.UPDATE)) {
    return '';
...
function deleteButton(record) {
  if (!rolePermission.hasOne(AppPermissions.DY.Shop.DELETE)) {
    return '';
  } else if (record.roleId === 1000) {
    return '';
  }
    ...
    >
      <template #extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :add-btn="rolePermission.hasOne(AppPermissions.DY.Shop.CREATE)"
          :delete-btn="rolePermission.hasOne(AppPermissions.DY.Shop.DELETE)"
          @add="handleAdd"
