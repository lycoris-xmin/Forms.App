...
import { fetchCreateDouyinShop, fetchUpdateDouyinShop } from '@/service/api';

defineOptions({
  name: 'DyShopOperateDrawer'
});
...
interface Props {
  ...
  /** the edit row data */
  rowData?: Api.Product.DYData | null;
}
...
async function submitHandler() {
  ...
  try {
    const { data: res, error } = isEdit.value
      ? await fetchUpdateDouyinShop({ ...model.value })
      : await fetchCreateDouyinShop(model.value.shopName);
