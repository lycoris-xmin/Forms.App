...
import { fetchAgainAliPlanTask, fetchGetAliPlanTaskAgainDetail } from '@/service/api';
...
watch(visible, async value => {
  if (value) {
    ...
    if (props.rowData) {
      ...
      try {
        const { data: res, error } = await fetchGetAliPlanTaskAgainDetail(props.rowData.id);
...
async function submitHandler() {
  ...
  const { data: res, error } = await fetchAgainAliPlanTask(json);
