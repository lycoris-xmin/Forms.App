...
import { fetchGetALIShopName, fetchGetTaobaoProductListForPlanTask } from '@/service/api';
...
const model = ref(createDetaltModel());
...
watch(checkedRowKeys, checkedKeys => {
  if (!model.value.multiple && checkedKeys.length > 1) {
    ...
    for (const item of _value) {
      ...
      if (index > -1) {
        ...
        value.value.push({
          ...
          skuList: item.selectedSkuMap ? JSON.parse(x.selectedSkuMap) : [],
          count: item.count,
          price: item.price,
          scope: item.scope,
          main: item.main
...
function createDetaltModel(multiple: boolean = false) {
  return {
    multiple,
    visible: false,
    products: [],
...
function multipleChangeHandler() {
  if (!model.value.multiple && model.value.products.length > 1) {
    model.value.products = [model.value.products[0]];
    checkedRowKeys.value = [model.value.products[0].id];
    rowSelection.value.selectedRowKeys = [model.value.products[0].id];
    rowSelection.value.selectedRows = rowSelection.value.selectedRows.filter(x => x.id === model.value.products[0].id);
  }
}
...
defineExpose({
  initSelectedProducts: () => {
    model.value = createDetaltModel(model.value?.multiple || false);
    remoSelectedKeys();
  }
});
</script>
...
<template>
  <p class="space-between v-center mb-12px flex">
    ...
    <ACheckbox v-model:checked="model.multiple" @change="multipleChangeHandler">多商品</ACheckbox>
  </p>
    ...
    <li v-for="(item, index) in model.products" :key="item.id">
      <div class="v-center mb-6px flex gap-8px">
        <ACheckbox
          ...
          @change="() => mainProductChangeHandler(index, item)"
        >
          主商品
        </ACheckbox>
  ...
  <div v-if="model.multiple || model.products.length === 0" class="actions mb-12px" @click="selectHandler">
    <p class="text-center">
      <PlusOutlined />
      添加商品
    </p>
