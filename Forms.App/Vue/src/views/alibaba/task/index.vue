...
import {
  ...
  fetchDeleteAliPlanTask,
  fetchExportAliPlanTask,
  fetchGetALIShopName,
  fetchGetAliPlanTaskList,
  fetchStopPlanTask
} from '@/service/api';
...
const {
  ...
  apiFn: (...args) => {
    return fetchGetAliPlanTaskList(...args);
  },
  apiParams: {
    pageIndex: 1,
    pageSize: 10
...
const { onBatchDeleted, handleEdit, onDeleted, checkedRowKeys, rowSelection } = useTableOperate(data, getData);

onMounted(() => {
  searchParams.platform = 40;
  getShopNameEnum();
});
...
async function deleteHandler(id: string) {
  const { data: res, error } = await fetchDeleteAliPlanTask([id]);
...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchDeleteAliPlanTask(ids);
    if (!error && res && res.code === 0) {
      onBatchDeleted();
    }
  }
            ...
            <div class="flex">
              ...
              <div class="plantask-info">
                <div class="plantask-title v-center flex">
                  <ATag color={platform.color}>1688联盟</ATag>
      ...
      customRender: ({ record }) => {
        ...
        return (
          <div style="font-size:14px">
            <p class="info mb-6px">
              <span class="text-right">1688号：</span>
            ...
            <p class="info mb-6px">
              <span class="text-right">本月牛气值：</span>
              <span title={record.currentMonth || '-'}>{record.currentMonth || '-'}</span>
            </p>
            <p class="info mb-6px">
              <span class="text-right">上月牛气值：</span>
              <span title={record.lastMonth || '-'}>{record.lastMonth || '-'}</span>
            </p>
            <p class="info mb-6px">
...
function detailButton(record) {
  ...
  if (rolePermission.hasOne(AppPermissions.Ali1688.Task.UPDATE)) {
    return '';
    if ([0, 10, 100].includes(record.status)) {
      return (
...
function deleteButton(record) {
  if (rolePermission.hasOne(AppPermissions.Ali1688.Task.DELETE) && authStore.userInfo.isTenant) {
...
function stopButton(record) {
  if (rolePermission.hasOne(AppPermissions.Ali1688.Task.UPDATE) && authStore.userInfo.isTenant) {
    // 已完成的任务不能删除
...
function againButton(record) {
  if (rolePermission.hasOne(AppPermissions.Ali1688.Task.CREATE) && authStore.userInfo.isTenant) {
    return (
...
function createHandler() {
  ...
  //
  // routerPushByKey('alibaba-task-create');
}
...
async function exportHandler() {
  ...
  try {
    const { data: res, error } = await fetchExportAliPlanTask(searchParams);
    if (!error && res && res.code === 0) {
    ...
    >
      <template #extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :add-btn="authStore.userInfo.isTenant && rolePermission.hasOne(AppPermissions.Ali1688.Task.CREATE)"
