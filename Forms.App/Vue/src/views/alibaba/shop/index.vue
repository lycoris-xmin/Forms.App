...
import { DeleteALIShop, GetALIShopList } from '@/service/api';
...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await DeleteALIShop(ids);
...
async function deleteHandler(id: string) {
  const { data: res, error } = await DeleteALIShop([id]);
...
function editButton(record) {
  if (!rolePermission.hasOne(AppPermissions.Ali1688.Shop.UPDATE)) {
    return '';
...
function deleteButton(record) {
  if (!rolePermission.hasOne(AppPermissions.Ali1688.Shop.DELETE)) {
    return '';
  } else if (record.roleId === 1000) {
    return '';
  }
    ...
    >
      <template #extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :add-btn="rolePermission.hasOne(AppPermissions.Ali1688.Shop.CREATE)"
          :delete-btn="rolePermission.hasOne(AppPermissions.Ali1688.Shop.DELETE)"
          @add="handleAdd"
