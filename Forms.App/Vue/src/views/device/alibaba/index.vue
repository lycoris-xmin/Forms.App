...
import { fetchDeviceAliInit, fetchGetDeviceAliList, fetchGetTenantPayTypeEnum } from '@/service/api';
import AppPermissions, { useRolePermission } from '@/hooks/custome/app.permissions';
import {
  deviceAliPayType as DEVICE_Ali_PAY_TYPE,
  deviceAliStatus as DEVICE_Ali_STATUS,
  gender as GENDER,
...
const payType = computed(() => {
  if (authStore.userInfo.isTenant) {
    return DEVICE_Ali_PAY_TYPE.filter(x => payTypeEnum.value.includes(x.value));
  }
  return DEVICE_Ali_PAY_TYPE;
});
...
function getColumns() {
  const cols = [
...

    {
      ...
      width: 100,
      customRender: ({ record }) => {
        const map = DEVICE_Ali_PAY_TYPE.filter(x => x.value === record.buyPayType)[0];
    ...
    },
    {
      ...
      width: 100,
      customRender: ({ record }) => {
        const map = DEVICE_Ali_STATUS.filter(x => x.value === record.status)[0];
        return <ATag color={map.color}>{map.label}</ATag>;
      }
    },
    {
...
function editButton(record) {
  if (!rolePermission.hasOne(AppPermissions.Device.Ali1688.UPDATE)) {
    return '';
  }
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        if (ids && ids.length) {
          const { data: res, error } = await fetchDeviceAliInit(ids);
  ...
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    >
      <template #extra>
        ...
        >
          <AButton
            v-if="rolePermission.hasOne(AppPermissions.Device.Ali1688.UPDATE) && authStore.userInfo.isTenant"
            size="small"
            ghost
            type="primary"
