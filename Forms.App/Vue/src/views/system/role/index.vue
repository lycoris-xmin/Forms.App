...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchDeleteRole(ids);
...
async function deleteHandler(id: string) {
  const { data: res, error } = await fetchDeleteRole([id]);
...
function editButton(record) {
  ...
  if (record.id === '1001' || record.id === '1002' || record.id === '1003') {
    return '';
  }
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <SearchFilter @search="getDataByPage(1)"></SearchFilter>
