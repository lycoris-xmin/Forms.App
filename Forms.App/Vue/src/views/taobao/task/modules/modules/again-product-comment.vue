...
const isMaxFileCount = computed(() => model.value.images.length + model.value.videos.length === 9);
...
function imageUplodaHandler() {
  if (!isMaxFileCount.value && model.value.videos.length < 9) {
...
async function videoFileChangeHandler(e) {
  if (e.target.files && e.target.files.length) {
    try {
      ...
      if (info.duration > 15) {
        window.$message?.warning('视频时长不能超过15秒');
        info.dispose();
        return;
      }
        ...
        <AListItem :key="item.id">
          ...
          <div v-if="item.comment.hasComment">
            ...
            <div class="wrap flex gap-8px">
              <div v-if="item.comment.images && item.comment.images.length">
                <div v-for="img in item.comment.images" :key="img" class="comment-preview">
                  <AImage :src="img.src" :height="80" :width="80"></AImage>
                </div>
              </div>
              <div v-else-if="item.comment.imageUrls && item.comment.imageUrls.length">
                <div v-for="imgUrl in item.comment.imageUrls" :key="imgUrl" class="comment-preview">
                  <AImage :src="imgUrl" :height="80" :width="80"></AImage>
                </div>
              </div>
              <div v-if="item.comment.videos && item.comment.videos.length" class="comment-preview">
              ...
              <div v-else-if="item.comment.videoUrls && item.comment.videoUrls.length" class="comment-preview">
                <img :src="item.comment.videoUrls[0]" :height="80" :width="80" />
                <div class="wrap v-center center flex gap-8px">
                  <p @click="() => videoPreviewHandler(item.comment.videoUrls[0])">
                    <EyeOutlined />
                    预览
                  </p>
                </div>
    ...
    <AForm ref="formRef" name="operate" layout="vertical" :model="model">
      ...
      <AFormItem>
        <template #label>
          <p>
            图片
            <small class="text-danger ml-4px">* 图片+视频最多9个，且图片必须小于2.1M</small>
          </p>
        </template>
        <input
          ref="imageUploadRef"
        ...
        <div class="wrap flex gap-8px">
          ...
          <div
            v-if="model.images.length < 9 && !isMaxFileCount"
            class="uploder center v-center flex"
            @click="imageUplodaHandler"
          >
            <PlusOutlined v-if="model.images.length < 9 && !isMaxFileCount" />
            <div v-else-if="model.images.length < 9" class="stop">
              <p class="text-center">
                <StopOutlined />
              </p>
      ...
      <AFormItem name="videos">
        <template #label>
          <p>
            视频
            <small class="text-danger ml-4px">* 视频最多一个，且视频时长不能超过15秒</small>
          </p>
        </template>
        <input
          ref="videoUploadRef"
