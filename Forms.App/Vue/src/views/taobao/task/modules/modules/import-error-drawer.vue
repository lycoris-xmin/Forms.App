...
import { apiUrl, fetchTaobaoImportErrorExportList } from '@/service/api';
...
function getAccountList(searchFilter: object) {
  ...
  return fetchTaobaoImportErrorExportList(searchFilter);
}
...
async function downloadExcel(exportFilePath, exportFileName) {
  try {
    const url = `${apiUrl}/api/plantask/excel/importerror?filePath=${exportFilePath}`;
...
<template>
  <ADrawer v-model:open="visible" title="失败统计报表（仅记录任务数据填写错误，不包含终止任务）" :width="1000">
