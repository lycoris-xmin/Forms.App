...
import {
  apiUrl,
  fetchDeleteTaobaoPlanTaskComment,
  fetchGetTaobaoPlanTaskCommentList,
  fetchRunRightNowTaobaoPlanTaskComment
} from '@/service/api';
...
const {
  ...
  apiFn: fetchGetTaobaoPlanTaskCommentList,
  apiParams: {
    pageIndex: 1,
    pageSize: 10
  },
...
async function batchDeleteHandler() {
  ...
  if (ids?.length) {
    const { data: res, error } = await fetchDeleteTaobaoPlanTaskComment(ids);
    if (!error && res?.code === 0) {
      onBatchDeleted();
    }
  }
...
async function deleteHandler(id: string) {
  const { data: res, error } = await fetchDeleteTaobaoPlanTaskComment([id]);
  if (!error && res?.code === 0) {
    onDeleted();
  }
}
...
function runRightNowButton(record: TableRecord) {
  if (rolePermission.hasOne(AppPermissions.Taobao.Comment.CREATE) && authStore.userInfo.isTenant) {
...
function deleteButton(record: TableRecord) {
  if (rolePermission.hasOne(AppPermissions.Taobao.Comment.DELETE)) {
...
async function runRightNowHandler(commentId: string) {
  try {
    const { data: res, error } = await fetchRunRightNowTaobaoPlanTaskComment(commentId);
    if (!error && res?.code === 0) {
