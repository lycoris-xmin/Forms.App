...
import { apiUrl, fetchCreateTaobaoProduct, fetchUpdateTaobaoProduct } from '@/service/api';

defineOptions({
  name: 'TaobaoProductOperateDrawer'
});
...
type Model = Pick<
  Api.Product.TaobaoData,
  'id' | 'itemId' | 'url' | 'title' | 'shopId' | 'image' | 'skuMap' | 'type'
> & {
  tab: string;
  fileList: Array;
  imagePriview: string;
...
function urlInputBlurHandler() {
  ...
  const url = new URL(model.value.url);

  model.value.itemId = url.searchParams.get('id') || '';
  model.value.url = `${url.protocol}//${url.host}${url.pathname}?id=${model.value.itemId}`;
}
...
function cancelSkuMapHandler(index) {
...
async function createOrUpdate() {
  ...
  try {
    ...
    const { data: res, error } =
      props.operateType === 'edit' ? await fetchUpdateTaobaoProduct(json) : await fetchCreateTaobaoProduct(json);
