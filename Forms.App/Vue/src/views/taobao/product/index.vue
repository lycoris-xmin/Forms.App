...
import { apiUrl, fetchDeleteTaobaoProduct, fetchGetTaobaoProductList, fetchGetTaobaoShopName } from '@/service/api';
...
const {
  ...
  apiFn: filter => {
    return fetchGetTaobaoProductList({ ...filter });
  },
  apiParams: {
    pageIndex: 1,
    pageSize: 10
...
async function getShopNameEnum(showToast = false) {
  try {
    const { data: res, error } = await fetchGetTaobaoShopName();
...
async function batchDeleteHandler() {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchDeleteTaobaoProduct(ids);
    if (!error && res && res.code === 0) {
      onBatchDeleted();
    }
  }
...
async function deleteHandler(id: string) {
  window.$modal?.confirm({
    ...
    async onOk() {
      const { data: res, error } = await fetchDeleteTaobaoProduct([id]);
...
function editButton(record) {
  if (!rolePermission.hasOne(AppPermissions.Taobao.Product.UPDATE)) {
    return '';
  }
...
function deleteButton(record) {
  if (!rolePermission.hasOne(AppPermissions.Taobao.Product.DELETE)) {
    return '';
  }
    ...
    >
      <template #extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :add-btn="rolePermission.hasOne(AppPermissions.Taobao.Product.CREATE)"
          :delete-btn="rolePermission.hasOne(AppPermissions.Taobao.Product.DELETE)"
          @add="handleAdd"
