...
import {
  fetchGetAlipayLovePayPayPerSettleList,
  fetchSettlementAlipayLovePayPerSettle
} from '@/service/api/modules/alipay-lovepay-record-per-settle';
...
const {
  ...
  apiFn: async (...filter) => {
    const res = await fetchGetAlipayLovePayPayPerSettleList(...filter);
...
function getColumns() {
  ...
  const otherCols = [
    ...
    },
    {
      ...
      width: 80,
      customRender: ({ record }) => {
        if (!authStore.userInfo.isTenantUser || !rolePermission.hasOne(AppPermissions.Report.AlipayLovePayPerSettle)) {
          return '';
        } else if (record.settlemented) {
          return '';
        } else if (record.userId !== authStore.userInfo.id) {
          return '';
        }
        return (
...
async function settlementHandler(id: string) {
  ...
  if (ids && ids.length) {
    const { data: res, error } = await fetchSettlementAlipayLovePayPerSettle(ids);
        ...
        >
          <AButton
            v-if="
              ...
              rolePermission.hasOne(AppPermissions.Report.AlipayLovePayPerSettle.UPDATE)
            "
            size="small"
            ghost
            type="primary"
            :disabled="checkedRowKeys.length === 0"
            @click="settlementHandler('')"
          >
            <template #icon>
              <icon-cryptocurrency-cny />
            </template>
