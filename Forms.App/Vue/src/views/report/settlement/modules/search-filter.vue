...
defineOptions({
  name: 'PlanTaskSettlementListSearch'
});
...
const model = defineModel<Api.PlanTaskSettlement.SearchFilter>('model', { required: true });
    ...
    >
      <ARow :gutter="[16, 16]" wrap>
        ...
        <ACol v-if="!props.userInfo.isTenant" :span="24" :md="12" :lg="6">
          <AFormItem label="收款商户" name="userId" class="m-0">
            <TenantSelect v-model:value="model.userId" placeholder="- 全部 -" />
          </AFormItem>
        </ACol>
        <ACol v-else-if="!props.userInfo.isTenantUser" :span="24" :md="12" :lg="6">
          <AFormItem label="收款员工" name="userId" class="m-0">
            <TenantSelect v-model:value="model.userId" placeholder="- 全部 -" :tenant-user="true" />
          </AFormItem>
        </ACol>
        <ACol v-if="!props.userInfo.isTenant" :span="24" :md="12" :lg="6">
          <AFormItem label="付款商户" name="pairUserId" class="m-0">
            <TenantSelect v-model:value="model.pairUserId" placeholder="- 全部 -" />
          </AFormItem>
        </ACol>
        <ACol v-else-if="!props.userInfo.isTenantUser" :span="24" :md="12" :lg="6">
          <AFormItem label="付款员工" name="pairUserId" class="m-0">
            <TenantSelect v-model:value="model.pairUserId" placeholder="- 全部 -" :tenant-user="true" />
          </AFormItem>
        </ACol>
        <div class="flex-1">
          <AFormItem class="m-0">
